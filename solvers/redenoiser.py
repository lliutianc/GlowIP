...
def GlowDenoiser(args):
    ...
    for gamma in loopOver:
        ...
        for i, data in enumerate(test_dataloader):
                ...
                try:
                    def closure():
                        ...
                        if not args.z_penalty_unsquared:
                            z_reg_loss_t= args.init_gamma * (z_sampled.norm(dim=1)**2).mean()
                        else:
                            z_reg_loss_t= args.init_gamma * z_sampled.norm(dim=1).mean()
                        loss_t = residual_t + z_reg_loss_t
                        global psnr
                        psnr = psnr_t(x_test, x_gen)
                        psnr = 10 * np.log10(1 / psnr.item())
                        print("\rStage 1: "
                              "At step=%0.3d|loss=%0.4f|residual=%0.4f|z_reg=%0.5f|psnr=%0.3f" %
            ...
            # Save original and noisy states.
